...
impl VisitMut for Unparen {
    fn visit_type_mut(&mut self, node: &mut syn::Type) {
        ...
        if let syn::Type::Paren(inner) = node {
            *node = (*inner.elem).clone();
        }
        if let syn::Type::Group(inner) = node {
            *node = (*inner.elem).clone();
        }
    }
}
...
fn text<T: ToTokens>(value: &T) -> String {
...
fn function(sig: &syn::Signature, owner: &str) -> Value {
    let ret = match &sig.output {
        ...
        syn::ReturnType::Type(_, t) => text(t),
    };
...
fn run(job: Job) -> Result<Value, String> {
    match job.mode.as_str() {
        ...
        "type" => {
            let mut ty: syn::Type = syn::parse_str(&job.text).map_err(|e| e.to_string())?;
            Unparen.visit_type_mut(&mut ty);
            Ok(tokens(quote!(#ty)))
        }
        "expr" => {
            let expr: syn::Expr = syn::parse_str(&job.text).map_err(|e| e.to_string())?;
            Ok(tokens(quote!(#expr)))
        }
...
fn main() {
    ...
    let result = serde_json::from_str::<Vec<Job>>(&raw)
        .map_err(|e| e.to_string())
        .and_then(|jobs| jobs.into_iter().map(run).collect::<Result<Vec<_>, _>>());
    match result {
        Ok(values) => println!("{}", serde_json::to_string(&values).unwrap()),
        Err(e) => {
            eprintln!("{e}");
            std::process::exit(2)
        }
