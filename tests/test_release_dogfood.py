...
def _write_artifact(
    ...
    *,
    ...
    if kind == "wheel":
        path = root / "agent_wiki_cli-1.8.0-py3-none-any.whl"
    ...
    path = root / "agent_wiki_cli-1.8.0.tar.gz"
    names = _artifact_paths("agent_wiki_cli-1.8.0/src/")
    if extra is not None:
        names.append(
            "agent_wiki_cli-1.8.0/src/llm_wiki_cli/skills/"
            f"wiki-reference/{extra}"
        )
    if internal_member is not None:
        names.append(f"agent_wiki_cli-1.8.0/{internal_member}")
