...
def test_committed_profile_freezes_intended_product_census():
    ...
    assert len(snapshot.all_source_paths) == 174
    assert {
        "src/llm_wiki_cli/extractors/python_bindings.py",
        "src/llm_wiki_cli/services/python_calls.py",
        "src/llm_wiki_cli/services/python_imports.py",
        "src/llm_wiki_cli/services/python_stdlib.py",
    } <= set(snapshot.language_paths("python"))
