...
def _validate_execution_manifest(payload: Mapping[str, Any]) -> None:
    ...
    if profile == "local_no_egress":
        ...
        try:
            from .broker import OciRuntimeConfig
    ...
    try:
        from .broker import validate_execution_manifest
    except ImportError:
        return
    try:
...
def _implementation_worktree_roots() -> tuple[Path, ...]:
    # This module lives one package level below the historical service module.
    # Resolve the same repository root so relocation cannot weaken the
    # protected-worktree boundary used by packet and cohort path validation.
...
_restore_legacy_definition_modules(
    globals(),
    legacy_module="llm_wiki_cli.services.documentation_calibration_controller",
)
