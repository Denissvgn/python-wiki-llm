...
3. **Check the hub.**
   ...
   Repeat the exact knowledge metadata, redaction profile, and public identity options if selected at export. Confirm `ok: true`, `0` issues, `0` warnings. An argv mismatch is not evidence for the selected hub even if the command exits successfully. This validates the generated mirror structurally without invoking mkdocs/docusaurus.
