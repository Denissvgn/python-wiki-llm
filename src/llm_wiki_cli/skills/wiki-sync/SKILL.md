...
6. **Review the diff before staging.** `git diff -- docs/llm_wiki/` — confirm only pages that correspond to the code diff changed; no reformatting or unrelated edits. `--dry-run` previews explicit surface initialization only; ordinary incremental sync still requires this post-write diff review. In `external_agent_docs`, compare the workspace baseline/diff and source/input hashes from the packet instead of requiring target Git state.
