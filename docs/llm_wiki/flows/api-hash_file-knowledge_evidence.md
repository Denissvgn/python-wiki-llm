...
sequenceDiagram
    ...
    participant p4 as path.read_bytes
    p0->>p1: sha256_bytes
    p1-->>p2: hashlib.sha256(…).hexdigest
    p1-->>p3: hashlib.sha256
    p0-->>p4: path.read_bytes
```
...
| From | To | Line | Call |
|---|---|---:|---|
| hash_file | sha256_bytes | 951 | `sha256_bytes(path.read_bytes(...))` |
...
| hash_file | path.read_bytes | 951 | `path.read_bytes(data not statically known)` |

### Boundary effects

| Kind | Target | Step | Line |
|---|---|---|---:|
| filesystem_read | `path.read_bytes` | `hash_file` | 951 |
