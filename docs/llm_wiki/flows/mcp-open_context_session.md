...
**Entry point:** `open_context_session` (`mcp`)
...
```mermaid
sequenceDiagram
    participant p0 as open_context_session
...
flowchart LR
    ...
    s6["6. TextContent"]
    s7["7. json.dumps"]
    s1 -->|"_native_tool_call(service.open_context_session)"| s2
...
|---|---|---|---|---|
| `open_context_session` | - | - | - | `_native_tool_call(...)` |
...
| From | To | Line | Call |
|---|---|---:|---|
| open_context_session | _native_tool_call | 1519 | `_native_tool_call(service.open_context_session)` |
...
Creates a bounded opaque handle only when the operator enables sessions. The handle belongs to this server and its configured workspace; it does not allow alternate roots or broader permissions.
