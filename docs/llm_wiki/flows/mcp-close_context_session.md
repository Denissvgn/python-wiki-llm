...
**Entry point:** `close_context_session` (`mcp`)
...
```mermaid
sequenceDiagram
    participant p0 as close_context_session
...
flowchart LR
    ...
    s6["6. TextContent"]
    s7["7. json.dumps"]
    s1 -->|"_native_tool_call(service.close_context_session, session_id)"| s2
...
|---|---|---|---|---|
| `close_context_session` | `session_id: str` | - | - | `_native_tool_call(...)` |
...
| From | To | Line | Call |
|---|---|---:|---|
| close_context_session | _native_tool_call | 1555 | `_native_tool_call(service.close_context_session, session_id)` |
...
Closes the identified server-owned session and releases its retained state. A closed or unknown handle cannot be used to recover prior workspace authority.
