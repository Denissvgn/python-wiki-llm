...
flowchart LR
    ...
    s1 -. "cast(CalleesResult, _run_query(...))" .-> s2
    s1 -->|"_run_query(...)"| s3
...
|---|---|---|---|---|
| `callees` | `symbol: object`, `service: DocumentationGraphQueryService \| None`, `src_dir: str`, `wiki_dir: str`, `limit: int`, `allow_external_src: bool`, `read_only: bool`, `source_selection: str \| Path \| None` | `CalleesResult` | - | `cast(...)` |
| `cast` | - | - | - | - |
...
| From | To | Line | Call |
|---|---|---:|---|
| callees | cast | 996 | `cast(CalleesResult, _run_query(...))` |
| callees | _run_query | 998 | `_run_query(...)` |
...
| callees | callees | 999 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).callees(symbol)` |
| callees | _query_service | 999 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection)` |
...
|---|---|---|---:|
| external_call | `callees` | `cast` | 996 |
| unresolved_call | `_run_query` | `callback` | 1314 |
| unresolved_call | `callees` | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).callees` | 999 |
| step_limit | `callees` | `first 12 steps` | 0 |
| truncated_flow | `callees` | `depth limit` | 0 |

## Behavior

This flow starts at `callees` and is classified as `api`. The generated call and data-flow sections are bounded static projections; runtime conditions and side effects require source-level confirmation.
