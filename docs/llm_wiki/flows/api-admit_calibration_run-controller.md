...
sequenceDiagram
    ...
    participant p21 as AssertionError (src/llm_wiki_cli/services….py:_require_exact_fields)
    ...
    p10-->>p21: AssertionError (src/llm_wiki_cli/services….py:_require_exact_fields)
    p10-->>p21: AssertionError (src/llm_wiki_cli/services….py:_require_exact_fields)
    p10->>p19: P0CalibrationSchemaError
    p10-->>p20: label.title (src/llm_wiki_cli/services….py:_require_exact_fields)
...
| admit_calibration_run | store.lock | 783 | `store.lock(data not statically known)` |
