# serialize_knowledge_index

**Entry point:** `serialize_knowledge_index` (`api`)
**Source:** [knowledge_model](../modules/knowledge_model.md)
**Modules touched:** [concept_identity](../modules/concept_identity.md), [knowledge_evidence](../modules/knowledge_evidence.md), [knowledge_governance](../modules/knowledge_governance.md), and 8 more

**Complete modules touched:**

- [concept_identity](../modules/concept_identity.md)
- [knowledge_evidence](../modules/knowledge_evidence.md)
- [knowledge_governance](../modules/knowledge_governance.md)
- [knowledge_graph](../modules/knowledge_graph.md)
- [knowledge_model](../modules/knowledge_model.md)
- [knowledge_reuse](../modules/knowledge_reuse.md)
- [markdown_sections](../modules/markdown_sections.md)
- [section_ownership](../modules/section_ownership.md)
- [validation](../modules/validation.md)
- [wiki_media](../modules/wiki_media.md)
- [wiki_surface](../modules/wiki_surface.md)

## Call sequence

<!-- Auto-generated from static call edges. Dashed arrows are external or unresolved calls. Reviewed runtime conditions and side effects belong in Behavior. -->
```mermaid
sequenceDiagram
    participant p0 as serialize_knowledge_index
    participant p1 as json.dumps
    participant p2 as knowledge_index_to_payload
    participant p3 as isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload)
    participant p4 as TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload)
    participant p5 as _emit_extensions
    participant p6 as isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions)
    participant p7 as _parse_extensions
    participant p8 as _object (src/llm_wiki_cli/services/knowledge_model.py)
    participant p9 as dict (src/llm_wiki_cli/services…nowledge_model.py:_object)
    participant p10 as require_mapping
    participant p11 as isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)
    participant p12 as key.encode
    participant p13 as KnowledgeModelError
    participant p14 as sorted (src/llm_wiki_cli/services…odel.py:_parse_extensions)
    participant p15 as _QUALIFIED_NAME_RE.fullmatch (src/llm_wiki_cli/services…odel.py:_parse_extensions)
    participant p16 as _child
    participant p17 as _normalize_json_value
    participant p18 as _normalize_json_value_inner
    participant p19 as isinstance (src/llm_wiki_cli/services…ormalize_json_value_inner)
    participant p20 as _string
    participant p21 as math.isfinite
    participant p22 as id
    p0-->>p1: json.dumps
    p0->>p2: knowledge_index_to_payload
    p2-->>p3: isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload)
    p2-->>p4: TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload)
    p2->>p5: _emit_extensions
    p5-->>p6: isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions)
    p5->>p7: _parse_extensions
    p7->>p8: _object (src/llm_wiki_cli/services/knowledge_model.py)
    p8-->>p9: dict (src/llm_wiki_cli/services…nowledge_model.py:_object)
    p8->>p10: require_mapping
    p10-->>p11: isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)
    p10-->>p11: isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)
    p10-->>p12: key.encode
    p8->>p13: KnowledgeModelError
    p8->>p13: KnowledgeModelError
    p8->>p13: KnowledgeModelError
    p7-->>p14: sorted (src/llm_wiki_cli/services…odel.py:_parse_extensions)
    p7-->>p15: _QUALIFIED_NAME_RE.fullmatch (src/llm_wiki_cli/services…odel.py:_parse_extensions)
    p7->>p13: KnowledgeModelError
    p7->>p16: _child
    p7->>p17: _normalize_json_value
    p17->>p18: _normalize_json_value_inner
    p18-->>p19: isinstance (src/llm_wiki_cli/services…ormalize_json_value_inner)
    p18->>p20: _string
    p18-->>p19: isinstance (src/llm_wiki_cli/services…ormalize_json_value_inner)
    p18-->>p19: isinstance (src/llm_wiki_cli/services…ormalize_json_value_inner)
    p18-->>p21: math.isfinite
    p18->>p13: KnowledgeModelError
    p18-->>p19: isinstance (src/llm_wiki_cli/services…ormalize_json_value_inner)
    p18-->>p22: id
```

> Call sequence diagram shows 30 of 1080 interactions; 1050 omitted to keep the visualization within the 30-interaction and generated-diagram limits.

> Trace truncated at the depth limit; deeper calls are omitted.

## Data flow

<!-- Auto-generated static analysis. Treat values and boundaries as best-effort hints, not runtime proof. -->
```mermaid
flowchart LR
    s1["1. serialize_knowledge_index"]
    s2["2. json.dumps"]
    s3["3. knowledge_index_to_payload"]
    s4["4. isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload)"]
    s5["5. TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload)"]
    s6["6. _emit_extensions"]
    s7["7. isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions)"]
    s8["8. _parse_extensions"]
    s9["9. _object (src/llm_wiki_cli/services/knowledge_model.py)"]
    s10["10. dict (src/llm_wiki_cli/services…nowledge_model.py:_object)"]
    s11["11. require_mapping"]
    s12["12. isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)"]
    s1 -. "json.dumps(knowledge_index_to_payload(...), indent=2, sort_keys=True, ensure_ascii=False, allow_nan=False)" .-> s2
    s1 -->|"knowledge_index_to_payload(model)"| s3
    s3 -. "isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload)(model, KnowledgeIndex)" .-> s4
    s3 -. "TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload)('model must be a KnowledgeIndex')" .-> s5
    s3 -->|"_emit_extensions({...}, model.extensions, 'extensions')"| s6
    s6 -. "isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions)(extensions, FrozenDict)" .-> s7
    s6 -->|"_parse_extensions(extensions, path)"| s8
    s8 -->|"_object (src/llm_wiki_cli/services/knowledge_model.py)(value, path)"| s9
    s9 -. "dict (src/llm_wiki_cli/services…nowledge_model.py:_object)(require_mapping(...))" .-> s10
    s9 -->|"require_mapping(…)"| s11
    s11 -. "isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)(value, Mapping)" .-> s12
    click s1 "../modules/knowledge_model.md"
    click s3 "../modules/knowledge_model.md"
    click s6 "../modules/knowledge_model.md"
    click s8 "../modules/knowledge_model.md"
    click s9 "../modules/knowledge_model.md"
    click s11 "../modules/validation.md"
```

### Step data

| Step | Inputs | Reads | Writes | Returns |
|---|---|---|---|---|
| `serialize_knowledge_index` | `model: KnowledgeIndex` | - | - | `...` |
| `json.dumps` | - | - | - | - |
| `knowledge_index_to_payload` | `model: KnowledgeIndex` | `KnowledgeIndex`, `KnowledgeModelError` | - | `_knowledge_index_to_payload_unchecked(...)` |
| `isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload)` | - | - | - | - |
| `TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload)` | - | - | - | - |
| `_emit_extensions` | `payload: dict[str, Any]`, `extensions: Extensions`, `path: str` | - | `payload[...]` | `payload` |
| `isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions)` | - | - | - | - |
| `_parse_extensions` | `value: object`, `path: str` | - | `result[...]` | `result` |
| `_object (src/llm_wiki_cli/services/knowledge_model.py)` | `value: object`, `path: str` | - | - | `dict(...)` |
| `dict (src/llm_wiki_cli/services…nowledge_model.py:_object)` | - | - | - | - |
| `require_mapping` | `value: object`, `error: Exception`, `require_string_keys: bool`, `key_error: Exception \| None`, `require_utf8_keys: bool`, `utf8_key_error: Exception \| None` | `Mapping` | - | `value` |
| `isinstance (src/llm_wiki_cli/services…dation.py:require_mapping)` | - | - | - | - |

### Call data

| From | To | Line | Call |
|---|---|---:|---|
| serialize_knowledge_index | json.dumps | 687 | `json.dumps(knowledge_index_to_payload(...), indent=2, sort_keys=True, ensure_ascii=False, allow_nan=False)` |
| serialize_knowledge_index | knowledge_index_to_payload | 688 | `knowledge_index_to_payload(model)` |
| knowledge_index_to_payload | isinstance (src/llm_wiki_cli/services…nowledge_index_to_payload) | 651 | `isinstance(model, KnowledgeIndex)` |
| knowledge_index_to_payload | TypeError (src/llm_wiki_cli/services…nowledge_index_to_payload) | 652 | `TypeError('model must be a KnowledgeIndex')` |
| knowledge_index_to_payload | _emit_extensions | 655 | `_emit_extensions({...}, model.extensions, 'extensions')` |
| _emit_extensions | isinstance (src/llm_wiki_cli/services…model.py:_emit_extensions) | 1976 | `isinstance(extensions, FrozenDict)` |
| _emit_extensions | _parse_extensions | 1977 | `_parse_extensions(extensions, path)` |
| _parse_extensions | _object (src/llm_wiki_cli/services/knowledge_model.py) | 1600 | `_object(value, path)` |
| _object (src/llm_wiki_cli/services/knowledge_model.py) | dict (src/llm_wiki_cli/services…nowledge_model.py:_object) | 1667 | `dict(require_mapping(...))` |
| _object (src/llm_wiki_cli/services/knowledge_model.py) | require_mapping | 1668 | `require_mapping(value, error=KnowledgeModelError(...), require_string_keys=True, key_error=KnowledgeModelError(...), require_utf8_keys=True, utf8_key_error=KnowledgeModelError(...))` |
| require_mapping | isinstance (src/llm_wiki_cli/services…dation.py:require_mapping) | 727 | `isinstance(value, Mapping)` |

### Boundary effects

*No boundary effects detected.*

### Static analysis gaps

| Kind | Step | Target | Line |
|---|---|---|---:|
| external_call | `serialize_knowledge_index` | `json.dumps` | 687 |
| external_call | `knowledge_index_to_payload` | `isinstance` | 651 |
| external_call | `knowledge_index_to_payload` | `TypeError` | 652 |
| external_call | `_emit_extensions` | `isinstance` | 1976 |
| external_call | `require_mapping` | `isinstance` | 727 |
| step_limit | `serialize_knowledge_index` | `first 12 steps` | 0 |
| truncated_flow | `serialize_knowledge_index` | `depth limit` | 0 |

## Behavior

This flow starts at `serialize_knowledge_index` and is classified as `api`. The generated call and data-flow sections are bounded static projections; runtime conditions and side effects require source-level confirmation.
