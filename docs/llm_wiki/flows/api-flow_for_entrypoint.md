...
| From | To | Line | Call |
|---|---|---:|---|
| flow_for_entrypoint | cast | 909 | `cast(FlowForEntrypointResult, _run_query(...))` |
| flow_for_entrypoint | _run_query | 911 | `_run_query(...)` |
...
| flow_for_entrypoint | flow_for_entrypoint | 912 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).flow_for_entrypoint(id_or_symbol)` |
| flow_for_entrypoint | _query_service | 912 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection)` |
...
|---|---|---|---:|
| external_call | `flow_for_entrypoint` | `cast` | 909 |
| unresolved_call | `_run_query` | `callback` | 1314 |
| unresolved_call | `flow_for_entrypoint` | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).flow_for_entrypoint` | 912 |
| step_limit | `flow_for_entrypoint` | `first 12 steps` | 0 |
| truncated_flow | `flow_for_entrypoint` | `depth limit` | 0 |

## Behavior

This flow starts at `flow_for_entrypoint` and is classified as `api`. The generated call and data-flow sections are bounded static projections; runtime conditions and side effects require source-level confirmation.
