...
| From | To | Line | Call |
|---|---|---:|---|
| runtime_consumed_inputs | isinstance | 1354 | `isinstance(source_snapshot, SourceSnapshot)` |
| runtime_consumed_inputs | TypeError | 1355 | `TypeError('source_snapshot must be a SourceSnapshot')` |
| runtime_consumed_inputs | isinstance | 1356 | `isinstance(generation_inputs, Mapping)` |
| runtime_consumed_inputs | TypeError | 1357 | `TypeError('generation_inputs must be a mapping')` |
| runtime_consumed_inputs | to_consumed_inputs | 1359 | `source_snapshot.to_consumed_inputs(data not statically known)` |
| runtime_consumed_inputs | _merge_explicit_consumed_input | 1361 | `_merge_explicit_consumed_input(consumed_by_path, path=plugin_lock_path, content_hash=plugin_lock_hash, kind=ConsumedInputKind.PLUGIN, field='plugin_lock')` |
| _merge_explicit_consumed_input | KnowledgeGenerationError | 1427 | `KnowledgeGenerationError(field, 'path and content hash must be supplied together')` |
| _merge_explicit_consumed_input | KnowledgeGenerationError | 1434 | `KnowledgeGenerationError(..., 'must be a non-empty repository-relative path')` |
| _merge_explicit_consumed_input | is_valid_sha256 | 1438 | `is_valid_sha256(content_hash)` |
...
|---|---|---|---:|
| unresolved_call | `runtime_consumed_inputs` | `isinstance` | 1354 |
| unresolved_call | `runtime_consumed_inputs` | `TypeError` | 1355 |
| unresolved_call | `runtime_consumed_inputs` | `isinstance` | 1356 |
| unresolved_call | `runtime_consumed_inputs` | `TypeError` | 1357 |
| unresolved_call | `runtime_consumed_inputs` | `source_snapshot.to_consumed_inputs` | 1359 |
