...
| From | To | Line | Call |
|---|---|---:|---|
| runtime_consumed_inputs | isinstance | 1386 | `isinstance(source_snapshot, SourceSnapshot)` |
| runtime_consumed_inputs | TypeError | 1387 | `TypeError('source_snapshot must be a SourceSnapshot')` |
| runtime_consumed_inputs | isinstance | 1388 | `isinstance(generation_inputs, Mapping)` |
| runtime_consumed_inputs | TypeError | 1389 | `TypeError('generation_inputs must be a mapping')` |
| runtime_consumed_inputs | to_consumed_inputs | 1391 | `source_snapshot.to_consumed_inputs(data not statically known)` |
| runtime_consumed_inputs | _merge_explicit_consumed_input | 1393 | `_merge_explicit_consumed_input(consumed_by_path, path=plugin_lock_path, content_hash=plugin_lock_hash, kind=ConsumedInputKind.PLUGIN, field='plugin_lock')` |
| _merge_explicit_consumed_input | KnowledgeGenerationError | 1459 | `KnowledgeGenerationError(field, 'path and content hash must be supplied together')` |
| _merge_explicit_consumed_input | KnowledgeGenerationError | 1466 | `KnowledgeGenerationError(..., 'must be a non-empty repository-relative path')` |
| _merge_explicit_consumed_input | is_valid_sha256 | 1470 | `is_valid_sha256(content_hash)` |
...
|---|---|---|---:|
| unresolved_call | `runtime_consumed_inputs` | `isinstance` | 1386 |
| unresolved_call | `runtime_consumed_inputs` | `TypeError` | 1387 |
| unresolved_call | `runtime_consumed_inputs` | `isinstance` | 1388 |
| unresolved_call | `runtime_consumed_inputs` | `TypeError` | 1389 |
| unresolved_call | `runtime_consumed_inputs` | `source_snapshot.to_consumed_inputs` | 1391 |
