...
flowchart LR
    ...
    s1 -. "cast(CallersResult, _run_query(...))" .-> s2
    s1 -->|"_run_query(...)"| s3
...
|---|---|---|---|---|
| `callers` | `symbol: object`, `service: DocumentationGraphQueryService \| None`, `src_dir: str`, `wiki_dir: str`, `limit: int`, `allow_external_src: bool`, `read_only: bool`, `source_selection: str \| Path \| None` | `CallersResult` | - | `cast(...)` |
| `cast` | - | - | - | - |
...
| From | To | Line | Call |
|---|---|---:|---|
| callers | cast | 967 | `cast(CallersResult, _run_query(...))` |
| callers | _run_query | 969 | `_run_query(...)` |
...
| callers | callers | 970 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).callers(symbol)` |
| callers | _query_service | 970 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection)` |
...
|---|---|---|---:|
| external_call | `callers` | `cast` | 967 |
| unresolved_call | `_run_query` | `callback` | 1314 |
| unresolved_call | `callers` | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).callers` | 970 |
| step_limit | `callers` | `first 12 steps` | 0 |
| truncated_flow | `callers` | `depth limit` | 0 |

## Behavior

This flow starts at `callers` and is classified as `api`. The generated call and data-flow sections are bounded static projections; runtime conditions and side effects require source-level confirmation.
