...
sequenceDiagram
    ...
    participant p17 as range (src/llm_wiki_cli/services…ve_table_description_cells)
    ...
    p0-->>p17: range (src/llm_wiki_cli/services…ve_table_description_cells)
...
| From | To | Line | Call |
|---|---|---:|---|
| preserve_table_description_cells | normalize_markdown(…).splitlines | 877 | `normalize_markdown(markdown).splitlines(data not statically known)` |
