...
flowchart LR
    ...
    s1 -. "cast(ConceptResult, _run_query(...))" .-> s2
    s1 -->|"_run_query(...)"| s3
...
|---|---|---|---|---|
| `get_concept` | `locator_or_exact_route: object`, `service: DocumentationGraphQueryService \| None`, `src_dir: str`, `wiki_dir: str`, `limit: int`, `allow_external_src: bool`, `read_only: bool`, `source_selection: str \| Path \| None` | `ConceptResult` | - | `cast(...)` |
| `cast` | - | - | - | - |
...
| From | To | Line | Call |
|---|---|---:|---|
| get_concept | cast | 1083 | `cast(ConceptResult, _run_query(...))` |
| get_concept | _run_query | 1085 | `_run_query(...)` |
...
| get_concept | get_concept | 1086 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).get_concept(locator_or_exact_route)` |
| get_concept | _query_service | 1086 | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection)` |
...
|---|---|---|---:|
| external_call | `get_concept` | `cast` | 1083 |
| unresolved_call | `_run_query` | `callback` | 1314 |
| unresolved_call | `get_concept` | `_query_service(service, src_dir=src_dir, wiki_dir=wiki_dir, limit=limit, allow_external_src=allow_external_src, read_only=read_only, source_selection=source_selection).get_concept` | 1086 |
| step_limit | `get_concept` | `first 12 steps` | 0 |
| truncated_flow | `get_concept` | `depth limit` | 0 |

## Behavior

This flow starts at `get_concept` and is classified as `api`. The generated call and data-flow sections are bounded static projections; runtime conditions and side effects require source-level confirmation.
