...
| `.python_imports` | `normalized_root`, `under_root` |
...
|-------|------|-------|-------------|
| [PackageInfo](../entities/PackageInfo.md) | 33 | — | Metadata for a single Python package discovered on disk. |

## Functions

| Function | Signature | Decorators | Description |
|----------|-----------|------------|-------------|
| `_configured_import_roots` | `(text: str, *, setup_py: bool = False) -> tuple[str, ...] \| None` | — | Read literal packaging source roots without executing a build backend. |
...
| `stamp_inventory_packages` | `(inventory: dict, packages: Sequence[PackageInfo], *, source_paths: Sequence[str] = ()) -> None` | — | Add a ``"package"`` key to each inventory entry in-place. |
| `_package_import_scope` | `(package: PackageInfo, source_paths: set[str]) -> dict` | — | — |
