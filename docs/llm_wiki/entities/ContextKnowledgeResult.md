# ContextKnowledgeResult

**Location:** `src/llm_wiki_cli/api_types.py:166`
**Kind:** Class
**Bases:** `_ContextKnowledgeRequired`
**Module:** [api_types](../modules/api_types.md)

## Description

Canonical explicit-mode knowledge outcome.

## Attributes

| Name | Type | Presence | Description |
|------|------|----------|-------------|
| `selection` | `ContextKnowledgeSelection` | *optional* | — |

## Methods

*No public methods. Inherits from base classes.*

## Relationships

<!-- Auto-generated relationship summary. Do not edit by hand. -->
```mermaid
flowchart LR
    n0["ContextKnowledgeResult (src/llm_wiki_cli/api_types.py)"]
    n1["_ContextKnowledgeRequired (src/llm_wiki_cli/api_types.py)"]
    n0 --> n1
    click n0 "../modules/api_types.md"
    click n1 "../modules/api_types.md"
```

### Summary

| Module | Methods | Attributes |
|---|---:|---|
| [api_types](../modules/api_types.md) | 0 | `selection` |

### Structure

| Kind | Entity | Module |
|---|---|---|
| Base | `_ContextKnowledgeRequired` | [api_types](../modules/api_types.md) |
