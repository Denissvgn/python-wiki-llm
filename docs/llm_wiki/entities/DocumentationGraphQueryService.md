...
**Location:** `src/llm_wiki_cli/services/documentation_queries.py:339`
**Kind:** Class
**Bases:** —
...
| `flow_for_entrypoint` | `(id_or_symbol: object) -> dict[str, Any]` | — | Return a bounded user-flow payload for an entry-point id or symbol. |
| `callers` | `(symbol: object) -> dict[str, Any]` | — | Return bounded callers for exactly one callable symbol. |
| `callees` | `(symbol: object) -> dict[str, Any]` | — | Return bounded callees for exactly one callable symbol. |
| `dependency_neighborhood` | `(path: object) -> dict[str, Any]` | — | Return bounded inbound/outbound dependency neighbors for a source path. |
| `data_flow_for_entrypoint` | `(id_or_symbol: object) -> dict[str, Any]` | — | Return a bounded data-flow payload for an entry-point id or symbol. |
| `pages_for_symbol` | `(symbol: object) -> dict[str, Any]` | — | Return bounded wiki surface pages related to exactly one symbol. |
| `get_concept` | `(locator_or_exact_route: object) -> dict[str, Any]` | — | Return one concept selected by current coordinate, UID, or alias. |
| `related_concepts` | `(locator_or_exact_route: object, *, direction: str = 'both', kinds: Optional[Iterable[str]] = None) -> dict[str, Any]` | — | Return bounded relationship observations for one exact identity. |
| `list_concept_sections` | `(locator_or_exact_route: object, *, ownership: str \| None = None) -> dict[str, Any]` | — | Return bounded document-order sections for one exact concept. |
| `traverse_typed_graph` | `(locator_or_exact_route: object, *, direction: str = 'both', kinds: Optional[Iterable[str]] = None, origins: Optional[Iterable[str]] = None, resolutions: Optional[Iterable[str]] = None, include_evidence: bool = False) -> dict[str, Any]` | — | Return a bounded traversal of the persisted typed-graph extension. |
| `explain_evidence` | `(locator_or_exact_route: object) -> dict[str, Any]` | — | Return full stored and computed evidence for one exact identity. |
...
flowchart LR
    ...
    n1["_query_service (src/llm_wiki_cli/api.py)"]
    n2["build_documentation_query_service (src/llm_wiki_cli/api.py)"]
    n3["callees (src/llm_wiki_cli/api.py)"]
    n4["callers (src/llm_wiki_cli/api.py)"]
    n5["data_flow_for_entrypoint (src/llm_wiki_cli/api.py)"]
    n6["dependency_neighborhood (src/llm_wiki_cli/api.py)"]
    n7["explain_evidence (src/llm_wiki_cli/api.py)"]
    n8["flow_for_entrypoint (src/llm_wiki_cli/api.py)"]
    n9["get_concept (src/llm_wiki_cli/api.py)"]
    n10["list_concept_sections (src/llm_wiki_cli/api.py)"]
    n11["pages_for_symbol (src/llm_wiki_cli/api.py)"]
    n12["related_concepts (src/llm_wiki_cli/api.py)"]
    n1 --> n0
    n2 --> n0
    n3 --> n0
    n4 --> n0
...
|---|---:|---|
| [documentation_queries](../modules/documentation_queries.md) | 49 | — |
