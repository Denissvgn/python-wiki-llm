# MarkdownContextResult

**Location:** `src/llm_wiki_cli/api_types.py:227`
**Kind:** Class
**Bases:** `TypedDict`
**Module:** [api_types](../modules/api_types.md)

## Description

Markdown rendering plus its source context payload.

## Attributes

| Name | Type | Presence | Description |
|------|------|----------|-------------|
| `content` | `str` | *required* | — |
| `payload` | `ContextPayload` | *required* | — |
| `warnings` | `list[str]` | *required* | — |

## Methods

*No public methods. Inherits from base classes.*

## Relationships

<!-- Auto-generated relationship summary. Do not edit by hand. -->
```mermaid
flowchart LR
    n0["MarkdownContextResult (src/llm_wiki_cli/api_types.py)"]
    n1["TypedDict"]
    n2["build_context (src/llm_wiki_cli/api.py)"]
    n0 --> n1
    n2 --> n0
    click n0 "../modules/api_types.md"
    click n2 "../modules/api.md"
```

### Summary

| Module | Methods | Attributes |
|---|---:|---|
| [api_types](../modules/api_types.md) | 0 | `content`, `payload`, `warnings` |

### Structure

| Kind | Entity | Module |
|---|---|---|
| Base | `TypedDict` | — |

### References

| Reference | Kind | Source | Call sites |
|---|---|---|---:|
| `build_context` | type_reference | [api](../modules/api.md) | — |
